...
def sanitize_filename(name: str) -> str:
    """Replace spaces/quotes/apostrophes with underscores for pandoc safety."""
    return (
        name.replace(" ", "_")
            .replace("'", "_")
            .replace("’", "_")
...
def convert_format(input_path: str, output_format: str, output_target: str = None):
    """Convert Markdown to other formats (html, txt, docx) with pandoc.
       - Single file: convert directly.
       - Folder: try batch mode. If it fails, ask user whether to fallback
         to per-file conversion or sanitize filenames for batch mode.
    """
    p = Path(input_path)
    ...
    if not files:
        print(f"[PANDOC] No markdown files in {input_path}")
        return

    # Handle output target
    out_path = Path(output_target) if output_target else None
    is_single_file_target = (
        out_path and out_path.suffix.lower() in [".txt", ".html", ".docx"]
    )

    # Single file conversion
    if is_single_file_target:
        if len(files) > 1:
            print("[PANDOC] Warning: multiple input files but single output specified. Only first will be written.")
        file = files[0]
        try:
            subprocess.run(
                ["pandoc", str(file), "-o", str(out_path)],
                check=True,
                capture_output=True,
            )
            print(f"[PANDOC] {file.name} -> {out_path}")
        except subprocess.CalledProcessError as e:
            print(f"[PANDOC ERROR] {file.name}: {e}")
        return

    # Folder conversion (batch mode first)
    if out_path:
        out_path.mkdir(parents=True, exist_ok=True)
        cmd = ["pandoc", "--to", output_format, "--output-dir", str(out_path)]
        cmd.extend(str(f) for f in files)
        try:
            subprocess.run(cmd, check=True, capture_output=True)
            print(f"[PANDOC] Batch converted {len(files)} file(s) -> {out_path}")
        except subprocess.CalledProcessError:
            print("[PANDOC] Batch conversion failed.")
            choice = input("Retry with [P]er-file conversion or [R]ename files? (P/R): ").strip().lower()
            if choice.startswith("p"):
                print("[PANDOC] Falling back to per-file conversion...")
                for file in files:
                    out_file = out_path / (file.stem + f".{output_format}")
                    try:
                        subprocess.run(
                ...
                print("[PANDOC] Sanitizing filenames and retrying batch mode...")
                safe_dir = out_path / "sanitized"
                safe_dir.mkdir(parents=True, exist_ok=True)
                safe_files = []
                for file in files:
                    safe_name = sanitize_filename(file.name)
                    safe_path = safe_dir / safe_name
                    safe_path.write_text(file.read_text(encoding="utf-8"), encoding="utf-8")
                    safe_files.append(safe_path)
                cmd = ["pandoc", "--to", output_format, "--output-dir", str(out_path)]
                cmd.extend(str(f) for f in safe_files)
                try:
                    subprocess.run(cmd, check=True, capture_output=True)
                    print(f"[PANDOC] Batch converted {len(safe_files)} sanitized file(s) -> {out_path}")
                except subprocess.CalledProcessError as e:
                    print(f"[PANDOC ERROR] Sanitized batch failed: {e}")
